// fen_to_tensor_wrapper.cpp
// Non-template implementation to avoid instantiation issues
...
std::deque<float> safe_fen_to_tensor(const std::string& fen) {
    ...
    if (!(fen_stream >> board_str >> turn_str >> castling_str >> ep_str)) {
        std::cerr << "[ERROR] Failed to parse FEN (required fields): " << fen << std::endl;
        return tensor;  // Return zero tensor on error
    }
    
    // Read optional fields (may not be present)
    fen_stream >> halfmove_str >> fullmove_str;
    ...
    for (char c : board_str) {
        if (c == '/') {
            ...
            if (rank < 0 || rank >= 8 || file < 0 || file >= 8) {
                std::cerr << "[ERROR] Index out of bounds in FEN parsing" << std::endl;
                continue;
            }
            ...
            if (piece_type_idx != -1) {
                ...
                // Bounds check
                if (tensor_idx >= 0 && tensor_idx < static_cast<int>(tensor.size())) {

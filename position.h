...
public:
	...
	// My own function to be able to set and get values for board pieces 
	// =====================================            ========================

	template<Color C> inline Bitboard pawns() const;
	template<Color C> inline Bitboard knights() const;
	template<Color C> inline Bitboard bishops() const;
	template<Color C> inline Bitboard rooks() const;
	template<Color C> inline Bitboard queens() const;
	template<Color C> inline Bitboard kings() const;
...
// =========================================     ===================================
	...
	template<Color Us>
	Move *generate_legals_captures(Move* list);
...
// Returns bitboard of all rooks

template<Color C> 
inline Bitboard Position::rooks() const {
	return C == WHITE ? piece_bb[WHITE_ROOK]:
		piece_bb[BLACK_ROOK];
}

// Returns bitboard of all pawns

template<Color C> 
inline Bitboard Position::pawns() const {
	return C == WHITE ? piece_bb[WHITE_PAWN]:
		piece_bb[BLACK_PAWN];
}

// Returns bitboard of all knights

template<Color C> 
inline Bitboard Position::knights() const {
	return C == WHITE ? piece_bb[WHITE_KNIGHT]:
		piece_bb[BLACK_KNIGHT];
}

// Returns bitboard of all bishops

template<Color C> 
inline Bitboard Position::bishops() const {
	return C == WHITE ? piece_bb[WHITE_BISHOP]:
		piece_bb[BLACK_BISHOP];
}

// Returns bitboard of all queens

template<Color C> 
inline Bitboard Position::queens() const {
	return C == WHITE ? piece_bb[WHITE_QUEEN]:
		piece_bb[BLACK_QUEEN];
}

// Returns bitboard of kings

template<Color C> 
inline Bitboard Position::kings() const {
	return C == WHITE ? piece_bb[WHITE_KING]:
		piece_bb[BLACK_KING];
}
...
Move* Position::generate_legals(Move* list) {
	...
	const Bitboard our_bishops = bishops<Us>();
	const Bitboard our_queen = queens<Us>();
	const Bitboard our_rooks = rooks<Us>();
	...
	//Non-pinned knight moves captures
	...
	//Non-pinned bishops captures
	b1 = our_bishops & not_pinned;
	while (b1) {
		s = pop_lsb(&b1);
	...
	//Non-pinned rooks captures
	b1 = our_rooks & not_pinned;
	while (b1) {
		s = pop_lsb(&b1);
		b2 = attacks<ROOK>(s, all);
		list = make<CAPTURE>(s, b2 & capture_mask, list);
	}
	//Non-pinned  queens captures
	b1 = our_queen & not_pinned;
	while (b1) {
		s = pop_lsb(&b1);
		b2 = attacks<QUEEN>(s, all);
		list = make<CAPTURE>(s, b2 & capture_mask, list);
	}
	
	//Non-pinned knight moves quiet
	b1 = bitboard_of(Us, KNIGHT) & not_pinned;
	while (b1) {
		s = pop_lsb(&b1);
		b2 = attacks<KNIGHT>(s, all);
...
//Generates all legal moves in a position for the given side. Advances the move pointer and returns it.
template<Color Us>
Move* Position::generate_legals_captures(Move* list) {
	constexpr Color Them = ~Us;
...
public:
	...
	const Move* begs() const { return list; }
...
//generate_legals() function directly. It can be iterated over.
template<Color Us>
class MoveListCaptures {
public:
	explicit MoveListCaptures(Position& p) : last(p.generate_legals_captures<Us>(list)) {}
